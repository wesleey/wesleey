...
USER_NAME = os.environ['USER_NAME']  # 'wesleey'
...
def recursive_loc(owner, repo_name, data, cache_comment):
    """
    Busca histórico de commits via paginação GraphQL, com retry e sem recursão.
    Conta apenas commits feitos pelo OWNER_ID.
    Se falhar após N tentativas, ignora o repositório e salva em skipped_repos.txt.
    """
    ...
    while True:
        ...
        for attempt in range(3):
            ...
            if request.status_code == 200:
                ...
                print(
                    f"[retry] {request.status_code} em {repo_name}, tentativa {attempt+1}/3")
                time.sleep(2 ** attempt)  # backoff exponencial
            else:
                force_close_file(data, cache_comment)
                raise Exception("recursive_loc() falhou",
                                request.status_code, request.text, QUERY_COUNT)
        else:
            # saiu sem sucesso -> ignora repo
            print(
                f"[skip] {repo_name} ignorado após {attempt+1} tentativas (status {request.status_code})")
            with open("skipped_repos.txt", "a") as f:
                f.write(f"{owner}/{repo_name}\n")
            return 0, 0, 0  # ignora este repo

        result = request.json()
        repo = result["data"]["repository"]

        # repositório vazio
        if not repo or repo["defaultBranchRef"] is None:
            return 0, 0, 0
...
if __name__ == '__main__':
    ...
    # e.g {'id': 'MDQ6VXNlcjU3MzMxMTM0'} and 2019-11-03T21:15:07Z for username 'wesleey'
    user_data, user_time = perf_counter(user_getter, USER_NAME)
    OWNER_ID, acc_date = user_data
    formatter('account data', user_time)
    age_data, age_time = perf_counter(
        daily_readme, datetime.datetime(2003, 8, 3))
    ...
    if OWNER_ID == {'id': 'MDQ6VXNlcjU3MzMxMTM0'}:  # only calculate for user wesleey
